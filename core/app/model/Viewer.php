...
public static function countAll(){
	$con = Database::getCon();
	$sql = "select count(*) as c  from sell";
	$c = 0;
	$query = $con->query($sql);
	while($r=$query->fetch_array()){				
		$c = $r['c'];
...
public static function countAllFromDay($day){
	$con = Database::getCon();
	$sql = "select count(*) as c  from sell where date(created_at)=\"$day\"";
	$c = 0;
	$query = $con->query($sql);
	while($r=$query->fetch_array()){				
		$c = $r['c'];
...
public static function countAllFromToday(){
	$con = Database::getCon();
	$sql = "select count(*) as c  from sell where date(created_at)=date(NOW())";
	$c = 0;
	$query = $con->query($sql);
	while($r=$query->fetch_array()){				
		$c = $r['c'];
